...
def createmodel():
    ...
    return model
tensorboard = TensorBoard(log_dir="logslo1/1",histogram_freq=0, write_graph=True, write_images=True)

estimator=KerasRegressor(build_fn=createmodel)
est=estimator.fit(X_train,Y_train,epochs= 20, batch_size= 160,callbacks=[tensorboard])
